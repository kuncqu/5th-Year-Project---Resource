function [F] = BackFaceCulling_opt(Vinf, alpha, beta, mainDir, STLname)
...
[F,V,N] = stlread(STLname); %binary
%[V, F, N] = import_stl_fast('GOCE_right_meas.stl',1);     %ascii
...
G = round(rand(length(F),3)*255);

old = cd(CadFolder);
print([STLname,'_backface'],'-dpng','-r0') %Salva immagine %for a lower
cd(old);
close Figure 1
...
% old = cd(CadFolder);
% imageData = export_fig([STLname,'_backface'],'-png'); %higher accurate analysis, high comp. time
% cd(old);
...
out = [a,histc(CDATA_FIN_NUMBER(:),a)]; % mi d� i numeri asssociati ai colori che compaiono nell'immagine e la loro frequenza (n pixel con quel colore)
...
disp(['Shape initialised in: ', num2str(toc(tstartback)),'s'])
